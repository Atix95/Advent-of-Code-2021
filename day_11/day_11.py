    ...
    for step in range(100):
        ...
        while len(unchecked_flashing_octopuses) > 0:
            ...
            for dx, dy in DIRECTIONS:
                if 0 <= x + dx < len(energy_levels[0]) and 0 <= y + dy < len(
                    energy_levels
                ):
        ...
        num_of_flashes += len(checked_flashing_octopuses)

    return num_of_flashes


if __name__ == "__main__":
    num_of_flashes = octopus_flashes(load_energy_level("day_11_input.txt"))

    print(f"The number of flashing octopuses is: {num_of_flashes}")
