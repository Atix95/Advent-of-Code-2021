...
def x_max(risk_levels):
    # Calculate the width of the cave
    return len(risk_levels[0]) - 1


def y_max(risk_levels):
    # Calculate the legnth of the cave
...
def find_path_with_lowest_risk(risk_levels):
    ...
    # returned.
    x_max_cave = x_max(risk_levels)
    y_max_cave = y_max(risk_levels)
    ...
    while len(nodes_to_visit) > 0:
        ...
        for neighbor_node in neighbors(current_node, x_max_cave, y_max_cave):
            ...
            neighbor_risk = (
                current_risk + risk_levels[neighbor_node[1]][neighbor_node[0]]
            )
            hq.heappush(
                nodes_to_visit,
...
if __name__ == "__main__":
    ...
    lowest_risk, path = find_path_with_lowest_risk(risk_levels)

    print(
        "\n".join(str(node) for node in path),
        f"\nThe lowest risk of all paths is: {lowest_risk}",
        "\nPath:\n" + visualize_path(risk_levels, path),
    )
